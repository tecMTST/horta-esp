...
class Modem(object):
    ...
    def initialize(self):
        ...
        if not self.uart:
            ...
            # Setup UART
            self.uart = UART(self.uart, baudrate=9600, rx=Pin(self.MODEM_TX_PIN), tx=Pin(self.MODEM_RX_PIN))
    ...
    def execute_at_command(self, command, data=None, clean_output=True):
        ...
        commands = {
                    ...
                    'checaconexao':  {'string':'AT+COPS?', 'timeout':3, 'end': 'OK'},
                    'conectaman':    {'string':'AT+COPS=4,2,"{}"'.format(data), 'timeout':10, 'end': 'OK'},
                    'checareg':      {'string':'AT+CREG?', 'timeout':3, 'end': None},
                    'iniciagprs':    {'string':'AT+SAPBR=3,1,"Contype","GPRS"', 'timeout':3, 'end': 'OK'}, # Appeared on hologram net here or below
                    'setaapn':       {'string':'AT+SAPBR=3,1,"APN","{}"'.format(data), 'timeout':3, 'end': 'OK'},
                    'setausuario':   {'string':'AT+SAPBR=3,1,"USER","{}"'.format(data), 'timeout':3, 'end': 'OK'},
                    'setasenha':     {'string':'AT+SAPBR=3,1,"PWD","{}"'.format(data), 'timeout':3, 'end': 'OK'},
                    'abregprs':      {'string':'AT+SAPBR=1,1', 'timeout':3, 'end': 'OK'},
                    'pegabear':      {'string':'AT+SAPBR=2,1', 'timeout':3, 'end': 'OK'},
                    'iniciatcp':     {'string':'AT+CIPSTART="TCP","mqtt.eclipseprojects.io","1883"', 'timeout':3, 'end': 'OK'}, #ou apenas OK no final
                    '''IDEAL
                    'iniciatcp':     {'string':'AT+CIPSTART={}'.format(data), 'timeout':3, 'end': 'CONNECT OK'},
                    '''
                    'enviatcp':      {'string':'AT+CIPSEND={},'.format(data), 'timeout':3, 'end': '>'},
                    'enviadado':     {'string':data, 'timeout':1, 'end': 'SEND OK'},
                    'fechatcp':      {'string':'AT+CIPCLOSE=1', 'timeout':3, 'end': 'CLOSE OK'}
        }
    ...
    def tcp_requesta(self, server, mode='PUB', data=None):
        ...
        # First, init and set http
        logger.debug('Http request step #1.1 (iniciatcp)')
        self.execute_at_command('iniciatcp')
        ...
        if mode == 'SUB':
            ...
            logger.debug('tcp request step #2.3 (enviatcp)')
            self.execute_at_command('enviatcp', len(data))
            print('enviou comando')
            logger.debug('tcp request step #2.4 (enviadado)')
            self.execute_at_command('enviadado', data)
